    ...
    for target in test_seq:
        ...
        for note in target_seq:
            path = (
                ...
                + note
                + "/"
                + random.choice(os.listdir("./notes/" + note + "/"))
            )
            play(path)
            ...
            if note == target:
                ...
                if display:
                    ...
                    if out == "C#":
                        ...
                        out = "Bb"
                    print(
                        colored("that was not the target note ({})".format(out), "red")
                    )
                else:
...
def run(test_seq):
    ...
    if len(sys.argv) == 1:
        sys.argv.append("b")
    speed = 2 if sys.argv[2] == "s" else 2.5
    run_seq(test_seq, speed=speed, display=True)
